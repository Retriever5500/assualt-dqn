...
class Memory:
    def __init__(self, max_num_transitions, mini_batch_size):
        self.q = deque([], max_num_transitions)
    ...
    def sample_mini_batch(self):
        if len(self.q) < self.mini_batch_size:
            raise Exception('Don\'t try to sample mini-batches while number of stored transitions < mini_batch_size')
        
        idxs = np.random.randint(0, len(self.q), self.mini_batch_size)
        batch = (self.q[idx] for idx in idxs)
        return batch